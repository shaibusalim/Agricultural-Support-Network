...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $noOFTractor = $_POST['No'];
    $typeOFTractor = $_POST['type'];
    $email = $_POST['email'];

    // Insert into database (change table/column names as per your database schema)
    $sql = "INSERT INTO tractor_owners(firstName, lastName, address, phone, location, No, type, email) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssss", $firstName, $lastName, $address, $phone, $location, $noOFTractor, $typeOFTractor, $email);
    ...
    if ($stmt->execute()) {
        ...
        // Failed insertion
        echo "Error adding farmer: " . $conn->error;
    }
                    ...
                    <div class="pcoded-inner-content">
                        <div class="main-body">
                            ...
                            <div class="card-header">
                                <h5>Add Tractor Owner</h5>
                            </div>

                            <form id="addFarmerForm" method="post" action="add_tractorOwner.php">
        <input type="text" name="firstName" placeholder="First Name">
        <input type="text" name="lastName" placeholder="Last Name">
        <input type="text" name="address" placeholder="Address">
        <input type="text" name="No" placeholder="no of Tractor">
        <input type="text" name="type" placeholder="Tractor Type">
        <input type="text" name="location" placeholder="location">
        <input type="text" name="phone" placeholder="Phone">
        <input type="email" name="email" placeholder="email">
        <input type="submit" value="Add Tractor Owner">
</form>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>
        <a href='edit_farmer.php?id=".$row['owner_id']."' class='btn btn-primary btn-sm'>Edit</a>
        <button onclick='deleteTractorOwner(".$row['owner_id'].")' class='btn btn-danger btn-sm'>Delete</button>
      </td>";
