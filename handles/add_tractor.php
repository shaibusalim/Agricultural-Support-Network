...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $condition = $_POST['conditions']; // New field for condition
    $availability = $_POST['availability']; // New field for availability
    $utilization = $_POST['utilization']; // New field for utilization
    ...
    if (move_uploaded_file($_FILES["tractor_image"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO tractors (brand, model, year, horsepower, description, image_url, `conditions`, availability, utilization)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param("sssssssss", $brand, $model, $year, $horsepower, $description, $imageURL, $condition, $availability, $utilization);
                        ...
                        <tbody>
                            ...
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>
                                            <a href='edit_farm.php?id=".$row['tractor_id']."' class='btn btn-primary btn-sm'>View</a>
...
function deletetractor(tractor_id) {
    if (confirm("Are you sure you want to delete this tractor?")) {
        ...
        xhr.open("GET", "delete_tractor.php?id=" + tractor_id, true);
        xhr.send();
    }
}
