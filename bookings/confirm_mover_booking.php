...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_SERVER['CONTENT_TYPE'] === 'application/json') {
    ...
    if (isset($requestData['notification_id']) && isset($requestData['booking_id'])) {
        ...
        $updateNotificationQuery = "UPDATE mover_notifications SET is_confirmed = 'Confirmed' WHERE notification_id = ?";
        $stmtUpdateNotification = $conn->prepare($updateNotificationQuery);

        // Update the booking status to confirmed
        $updateBookingQuery = "UPDATE mover_bookings SET status = 'Confirmed' WHERE booking_id = ?";
