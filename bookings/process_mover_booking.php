...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insertQuery = "INSERT INTO mover_bookings (customer_name, customer_email, customer_phone, provider_id, provider_type, booking_date, booking_time, additional_requests) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($insertQuery);
    $stmt->bind_param("sssiisss", $customerName, $customerEmail, $customerPhone, $providerId, $providerType, $bookingDate, $bookingTime, $additionalRequests);
    ...
    if ($stmt->execute()) {
        ...
        $insertNotificationQuery = "INSERT INTO mover_notifications (mover_id, notification_text, booking_id, booking_date, booking_time, additional_requests) VALUES (?, ?, ?, ?, ?, ?)";
