...
session_start();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['tractor_id'])) {
        ...
        $sql = "SELECT * FROM tractors WHERE tractor_id = ? AND owner_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ii", $tractorId, $ownerId);
        $stmt->execute();
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<h1 class='tractor-title'>" . $row['brand'] . " " . $row['model'] . "</h1>";
            echo "<p class='tractor-info'>Year: " . $row['year'] . "</p>";
            echo "<p class='tractor-info'>Description: " . $row['description'] . "</p>";
            echo "<p class='tractor-info'>Condition: " . $row['conditions'] . "</p>";
            ...
            echo "Tractor details not found.";
        }
        $stmt->close();
        $conn->close();
    }
...
<?php
         ...
         if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['tractor_id'])) {
             $tractorId = $_GET['tractor_id'];
             $ownerId = $_SESSION['owner_id']; // Assuming you need the farmer's ID
             ...
             if ($result->num_rows > 0) {
                 ...
                 echo '<input type="hidden" name="tractor_id" value="' . $tractorId . '">';
                 ...
                 echo "Tractor not found or unauthorized access.";
             }
         
             $stmt->close();
             $conn->close();
         } else {
             echo "Tractor ID not provided or invalid request.";
         }
         ?>
...
// Check if the session variable is set
    ...
    if (isset($_GET['tractor_id'])) {
       $tractorId = $_GET['tractor_id'];
        ...
        $sql = "SELECT * FROM maintenance_records WHERE tractor_id=?";
        $stmt = $conn->prepare($sql);
