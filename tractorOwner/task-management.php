...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['owner_id'])) {
    ...
    $ownerId = $_SESSION['owner_id']; // Assuming you have a user session
    echo "Owner ID: " . $_SESSION['owner_id'];

    if(isset($ownerId)){
    $sql = "INSERT INTO owner_tasks (task_name, task_description, task_date, owner_id) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $taskName, $taskDescription, $taskDate, $ownerId);
...
if(isset($_SESSION['owner_id'])){
    $moverId = $_SESSION['owner_id']; 

$sql = "SELECT * FROM owner_tasks WHERE owner_id = ?";
$stmt = $conn->prepare($sql);
